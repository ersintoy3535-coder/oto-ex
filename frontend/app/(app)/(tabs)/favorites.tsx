...
export default function FavoritesScreen() {
  ...
  const renderItem = ({ item, index }: { item: Report; index: number }) => (
    ...
    >
      ...
      <View style={[styles.scoreBadge, { borderColor: scoreColor(item.guven_skoru) }]}>
        <Text style={[styles.scoreValue, { color: scoreColor(item.guven_skoru) }]}>{item.guven_skoru}</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  tabText: { color: colors.onSurfaceTertiary, fontSize: 13 },
  tabTextActive: { color: colors.brand, fontWeight: '500' },
  center: { flex: 1, alignItems: 'center', justifyContent: 'center', gap: spacing.md, padding: spacing.xl },
  emptyTitle: { color: colors.onSurface, fontSize: 16, fontWeight: '500' },
  ...
  scoreValue: { fontSize: 16, fontWeight: '500' },
});
